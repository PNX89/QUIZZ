...
Every number in here is real. 2024 Q2 real output was first published in the August 2024
vintage at 22918.7, restated to 22924.9 a month later, to 23223.9 the month after that, and to
23286.5 in the annual revision of October 2025. An agent asked what the figure was in September
2024 has exactly one correct answer, and it is not the one on the publisher's website today.
"""
...
def test_the_answer_is_what_was_published_at_the_knowing_time(
    ...
    result = answer_as_of(db, "ROUTPUT", "2024-Q2", as_of)
    assert isinstance(result, Answer)
    assert result.value == expected
    assert result.vintage == as_of
...
def test_a_later_revision_does_not_leak_backwards(db: sqlite3.Connection) -> None:
    ...
    answer 23286.5 here, and would be wrong by 368 billion dollars in a way nothing about the
    response would reveal.
    """
    result = answer_as_of(db, "ROUTPUT", "2024-Q2", "2024-09")
    assert isinstance(result, Answer)
    assert result.value == 22924.9
    assert result.value != 23286.5
...
def test_the_answer_carries_the_vintage_it_came_from(db: sqlite3.Connection) -> None:
    ...
    result = answer_as_of(db, "ROUTPUT", "2024-Q2", "2025-06")
    assert isinstance(result, Answer)
    assert result.vintage == "2024-10"
    assert result.vintage != result.as_of


def test_a_period_not_yet_published_is_not_an_answer(db: sqlite3.Connection) -> None:
    """July 2024 is before the first estimate of the quarter that had just ended."""
    result = answer_as_of(db, "ROUTPUT", "2024-Q2", "2024-07")
...
def test_not_published_is_a_different_outcome_from_a_refusal(db: sqlite3.Connection) -> None:
    ...
    """
    unpublished = answer_as_of(db, "ROUTPUT", "2024-Q2", "2024-07")
    refused = answer_as_of(db, "ROUTPUT", "2024-Q2", None)
...
def test_revised_since_is_true_only_when_a_later_vintage_disagrees(
    ...
    early = answer_as_of(db, "ROUTPUT", "2024-Q2", "2024-08")
    settled = answer_as_of(db, "ROUTPUT", "2024-Q2", "2026-08")
...
def test_the_newest_vintage_at_or_before_is_used_and_not_the_newest_there_is(
    ...
    """
    parameters = {"series": "ROUTPUT", "observation": "2024-Q2", "as_of": "2024-09"}
    bounded = db.execute(
        "select value from observations where series = :series and observation = :observation "
        "and vintage <= :as_of order by vintage desc limit 1",
        parameters,
    ).fetchone()
    ...
    assert bounded["value"] == 22924.9
    assert unbounded["value"] == 23286.5
    correct = answer_as_of(db, "ROUTPUT", "2024-Q2", "2024-09")
...
REFUSABLE = [
    ...
    pytest.param({"as_of": "2018-06"}, id="a knowing-time before the corpus opens"),
...
def test_every_refusal_is_byte_identical(db: sqlite3.Connection, call: dict[str, str]) -> None:
    ...
    """
    result = answer_as_of(db, "ROUTPUT", "2024-Q2", **call)
...
def test_a_held_out_period_is_refused_at_every_knowing_time(db: sqlite3.Connection) -> None:
    for as_of in ("2025-08", "2026-01", "2026-08"):
        result = answer_as_of(db, "ROUTPUT", "2025-Q3", as_of)
        assert isinstance(result, Refused)
        assert result.message == REFUSAL
...
def test_the_refusal_cannot_be_used_to_map_the_holdout(db: sqlite3.Connection) -> None:
    ...
    """
    held_out = answer_as_of(db, "ROUTPUT", "2025-Q3", "2026-08")
    out_of_range = answer_as_of(db, "ROUTPUT", "2024-Q2", "2018-06")
...
def test_an_unknown_series_raises_rather_than_refusing(db: sqlite3.Connection) -> None:
    ...
    with pytest.raises(ValueError, match="unknown series"):
        answer_as_of(db, "NOSUCH", "2024-Q2", "2026-01")
...
def test_an_unknown_period_is_refused_rather_than_answered_as_unpublished(
    ...
    """
    assert isinstance(answer_as_of(db, "ROUTPUT", "1801-Q1", "2026-01"), Refused)
